...
def weightedSum(w,m,dend,perc):#somma pesata per ogni livello
    s=[]
    for i in range(perc):
        somma=0
        for j in range(dend):
...
def actived(s):#funzione di attivazione
    y=[]
    for somma in s:
        y.append(1/(1+math.exp(-somma)))
    return y

def Dactived(t):#derivata funzione attivazione
    res=1/(1+math.exp(-t))
    res=res*(1-res)
    return res

def map_in_out(y1):#mappatura output strato 1 in input strato 2
    m2=[]
    m2.append(1)
    for i in y1:
        m2.append(i)
...
def calibrate_outlayer(w,rate,y,d,m,perc,dend,s):#calibrazione pesi strato esterno
    for i in range(perc):
        for j in range(dend):
            w[i*dend+j]+=rate*2*(d[i]-y[i])*m[j]*Dactived(s[i])
    return w

def calibrate_deeplayer(l1,rate,l2):#calibrazione pesi strato interno 
...
def RN(layer1,layer2):#richiamo rete neurale
